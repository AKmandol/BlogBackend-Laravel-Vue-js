        ...
        >
          ...
          <div class="_overflow _table_div  blogPost">
           ...
           <div class="_input_field">
             ...
             <span class="text-dark mr-3">Old Title Image:</span>
          ...
          <div class="_text_center">
            ...
            :disabled="isCreating"
            @click="updateContent">
            <Icon type="md-send"/> {{isCreating ? 'Please Wait' :' Update Blog Post'}}
          </Button>
          </div>
...
export default {
  ...
  methods: {
    ...
    //   console.log(this.data);
      ...
      const res = await this.callApi('post', `/app/update-blog/${id}`, this.data);
      if(res.status === 200) {
        this.success('Data Updated Succesfully');
  ...
  async created() {
    ...
    const [blog,cat, tag] = await Promise.all([
      this.callApi("get", `/app/get_single_blog/${id}`),
      this.callApi("get", "/app/get_category"),
      this.callApi("get", "/app/get_tags"),
    ])
    if (cat.status == 200) {
      this.blogLists = blog.data;
      ...
      this.editorData = blog.data.post;
